...
| `custom_tooltip.lua` | Per-cwd workspace tooltip: floating note top-right with rounded `DiagnosticInfo`-blue border (winblend 10, zindex 150); `<leader>xt` toggle, `<leader>xq` edit (default text "Type `<leader>xq` to edit"); injected via `AstroNvim/astrocore` opts `mappings` |
